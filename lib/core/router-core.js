    ...
    RouterCore.prototype.parseMetaData = function (metaData, router) {
        ...
        */
        if (data && (data.method || data.path)) {
            var route = (_a = data.path) !== null && _a !== void 0 ? _a : data.method;
            router.registerRoute(data.method, route, function (req, res) { });
        }
        else {
    ...
    RouterCore.prototype.showInfo = function (host, port) {
        ...
        for (var _i = 0, _a = this.routesInfo; _i < _a.length; _i++) {
            ...
            routeInfoTable.push({
                ...
                url: "http://".concat(host, ":").concat(port.port).concat(info.fullRoute)
            });
        }
